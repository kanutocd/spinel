...
- **残 ERR=44 / FAIL=22 の構造的ブロッカー**:
  1. **proc/closure の mrb_int ABI(closure-capture ~5 ERR + proc 6→0 等 FAIL)**: `sp_proc_call` の戻り値・args が mrb_int 固定。poly 値が proc 境界で truncate。poly-cell(sp_RbVal セル+`sp_cell_scan_rbval`)は実装途中まで行ったが、proc ABI 全体が mrb_int 前提なので **ABI を sp_RbVal 化する大改修が必要**(別案件、incremental coercion では不可)。試行ブランチ無し=未コミット破棄済。

...
- **残 ERR=60 / FAIL=21**: B4 builtin-arg coerce の残り = **散在 one-off**(`sp_bigint_new_int(poly)`→本来 `sp_poly_as_bigint`、codegen_stmt.c:510/563/634 の bigint-cell arm / `sp_IntArray_index_poly` arg2 / user `<=>`=`sp_*__lt_set_gt`(構造的に `expected expression before sp_RbVal` も伴う)/ `sp_collect_s` / bound-method `.call` が raw arg を widen poly param へ=codegen_call の bound-method dispatch)。closure-capture front-end(5)。FAIL=21(出力差、proc 6→0 等)。

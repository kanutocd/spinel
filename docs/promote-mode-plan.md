...
- **残 ERR=44 / FAIL=22 の構造的ブロッカー**:
  1. **proc/closure の poly 対応(closure-capture ~5 ERR + proc 6→0 等 FAIL)— ★(A) 単独 land 不可を実証(2026-06-21)**:
     ...
     - **正しい実装(未着手)= (i) widen 後に proc_ret 伝播 fixpoint を再実行**(全 3 サイト + ret_proc_ret を本体 ret から再導出して収束)、**かつ (ii) side-channel の再入性も確認**(naive widen 時の 920 退行は①が主因と判明したので (B) 単独要否は再評価要)、**+ (A) poly-cell + args int-slot 公開**。**3 点を一括**。perf: 各 proc 呼びの boxing 増、optcarrot gate 必須。
     - **代替(よりクリーン)= proc ABI を `sp_RbVal` 化**(`_proc_N(...)->sp_RbVal`、side-channel 全廃、`sp_proc_call`/compose/curry/bound-method の足並み)。proc_ret 分岐自体が消え fixpoint 不整合が原理的に起きない。perf 影響(sp_RbVal[16] スタック)を optcarrot で測定。**これが本命の恒久解。**

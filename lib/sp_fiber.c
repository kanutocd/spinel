 ...
 * See sp_fiber.h. The collector roots, the poly-hash used for fiber-local
 * storage, sp_gc_alloc, and sp_raise_cls live in the generated TU and are
 * reached by name. */
#include "sp_fiber.h"
#include <stdlib.h>
#include <string.h>
...
/* Completes the forward-declared tag so the helper prototypes below can
   spell the pointer type the original bodies use. */
...
sp_SymPolyHash *sp_SymPolyHash_new(void);
sp_SymPolyHash *sp_SymPolyHash_dup(sp_SymPolyHash *h);
sp_RbVal sp_SymPolyHash_get(sp_SymPolyHash *h, sp_sym k);
void sp_SymPolyHash_set(sp_SymPolyHash *h, sp_sym k, sp_RbVal v);
...
sp_RbVal sp_Fiber_storage_get(sp_Fiber*f,sp_sym k){if(!f->storage)return sp_box_nil();return sp_SymPolyHash_get(f->storage,k);}
void sp_Fiber_storage_set(sp_Fiber*f,sp_sym k,sp_RbVal v){if(!f->storage)f->storage=sp_SymPolyHash_new();sp_SymPolyHash_set(f->storage,k,v);}
